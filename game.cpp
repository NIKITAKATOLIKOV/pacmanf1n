...
//подгрузка текстур
sf::Font font;                               // шрифт
sf::Text level_text("", font, 30), score_text("", font, 30),lifes_text("", font, 30),
        fps_text("", font, 20), Bstart("START", font, 70), Bcontrol("CONTROL", font, 70),
        Bexit("EXIT", font, 70);
...
{// изначальное заполнение объектов (одноразовая)
    ...
    sf::Image Ipacman_0, Ipacman_1, Ipacman_2, Ipacman_3,
            ...
            Ienergy_2.loadFromFile("../data/images/Energy2.png"), Ienergy_3.loadFromFile("../data/images/Energy3.png"),
            Ienergy_4.loadFromFile("../data/images/Energy4.png"),
            ...
            IdeathP_6.loadFromFile("../data/images/Death6.png"), IdeathP_7.loadFromFile("../data/images/Death7.png"),
            IdeathP_8.loadFromFile("../data/images/Death8.png"),
            ...
            IdeathP_6.createMaskFromColor(sf::Color::Black), IdeathP_7.createMaskFromColor(sf::Color::Black),
            IdeathP_8.createMaskFromColor(sf::Color::Black),
            Icontrol.createMaskFromColor(sf::Color::Yellow), Iescape.createMaskFromColor(sf::Color::Yellow),
            Imenu.createMaskFromColor(sf::Color::Black);        TPacman_0.loadFromImage(Ipacman_0), TPacman_1.loadFromImage(Ipacman_1),
    ...
    for (short i = 0; i < 29; ++i)
    {
        ...
        {
            map[i][j] > 0 ? (dec_arr[i].push_back(false), dis_arr[i].push_back(100)) : (dec_arr[i].push_back(
                    true), dis_arr[i].push_back(-1));
        }
    }
}

void Game::Setup(bool isNewGame, bool isBegin)
{// новая игра
    short i = 0, j = 0;
    time_passed = killed_count = second_scared = second_deathP = second_fruit = char(0),
    second_pause = isBegin ? char(2) : char(0), isPause = deathPacman = gameOver = false, GRedX = 12, GRedY = 10,
    number_fruit = char((level + 2) > 21 ? rand() % 7 : short(level + 1) % 7);
    pacman = Pacman(14, 22); SPacman.setTextureRect(sf::IntRect(0, 0, 30, 30)),SPacman.setRotation(0.f);
    ghosts.clear();
    ghosts.push_back(Ghost(pacman, GhostType::Red, 12, 10));
    ghosts.push_back(Ghost(pacman, GhostType::Blue, 15, 10));
    ghosts.push_back(Ghost(pacman, GhostType::Orange, 15, 13));
    ghosts.push_back(Ghost(pacman, GhostType::Pink, 12, 13));
    for (; i < 4; ++i)
    {
        ghosts[i].setDir_Vec(
                Find_Path(dec_arr, dis_arr, ghosts[i].getX(), ghosts[i].getY(), pacman.getX(), pacman.getY(), Direction::Stop));
    }
    if (isNewGame)
    {
        ...
        for (i = 0; i < 4; ++i)
        {
            ghosts[i].setSpeed((level + 1) % 7 == 0 ? (level + 1) / 7 > 3 ? 2.25f : ghosts[i].getSpeed() + 0.25f : ghosts[i].getSpeed());
        }
        for (i = 0; i < mapHeight; ++i)
        {
            for (short j = 0; j < 28; ++j)
            {
                energy[i][j] = energy_def[i][j];
            }
        }
        switch (number_fruit)
        {
...
{// рисовка
    ...
    win.draw(SMap), second_pause != 4 ? win.draw(level_text) : void(),
            win.draw(score_text), win.draw(Spacman), win.draw(lifes_text),
            win.draw(fps_text), win.draw(SFruit);
    if (second_fruit != char(0))
    {
        SFruit.setPosition(352.25f, 408.f); win.draw(SFruit);
    }
    for (; i < 812; ++i)
    {
    ...
    pacman.draw(win, Spacman, TPacman_0, TPacman_1, TPacman_2, TPacman_3, TDeathP_0, TDeathP_1, TDeathP_2, TDeathP_3,
                TDeathP_4, TDeathP_5, TDeathP_6, TDeathP_7, time_passed, second_deathP, lifes, deathPacman, gameOver, isPause);
    if (time_passed >= 600 && second_deathP == 1)
    {
        Setup(false);
    }
    for (i = 0; i < 4; ++i)
    {
        ghosts[i].draw(win, dec_arr, dis_arr, TGhostE, TGhostN_0, TGhostN_1, TGhostS_0, TGhostS_1, time_passed, second_scared, killed_count);
    }
}
void Game::Control(sf::RenderWindow& win)
{
    sf::Sprite SControl(TControl), SEscape(TEscape), SSpace(TSpace);
    sf::Text Bback("BACK", font, 70), control("CONTROL", font, 30),
            escape("OPEN THE MENU", font, 30), space("PAUSE", font, 30);
    Bback.setPosition(250.f, 600.f), control.setPosition(280, 100), escape.setPosition(230, 300),
            space.setPosition(300, 450), SControl.setPosition(180, 150), SEscape.setPosition(320, 350),SSpace.setPosition(200, 500);
    while (win.isOpen())
    {
        sf::Event event;
        while (win.pollEvent(event))
        {
            if (event.type == sf::Event::KeyPressed)
            {
                ...
                {
                    SMenuP.setPosition(-30.f, -30.f);
                    return;
                }
                if (event.key.code == sf::Keyboard::Enter)
                {
                    if (Bback.getFillColor() == sf::Color::Yellow)
                    {
                        SMenuP.setPosition(-30.f, -30.f);
                        return;
                    }
            ...
            if (sf::IntRect(250, 600, 200, 70).contains(sf::Mouse::getPosition(win)))
            {

                Bback.getFillColor() != sf::Color::Yellow ? SMenuP.setPosition(210.f, 630.f): void(), Bback.setFillColor(sf::Color::Yellow);
            }
            else
            {
                SMenuP.setPosition(-30.f, -30.f), Bback.setFillColor(sf::Color::White);
            }
...
void Game::SemiMenu(sf::RenderWindow& win)
{
    ...
    while (win.isOpen())
    {
        ...
        while (win.pollEvent(event))
        {
            if (event.type == sf::Event::KeyPressed)
            {
                ...
                if (event.key.code == sf::Keyboard::Enter)
                {
                    if (Bcontinue.getFillColor() == sf::Color::Yellow)
                    {
                        SMenuP.setPosition(-30.f, -30.f);
                        return;
                    }
                    if (Bcontrol.getFillColor() == sf::Color::Yellow)
                    {
                        SMenuP.setPosition(-30.f, -30.f), Control(win);
                    }
                    if (Bmenu.getFillColor() == sf::Color::Yellow)
                    {
                        isEscape = true;
                        SMenuP.setPosition(-30.f, -30.f);
                        return;
            ...
            if (sf::IntRect(170, 300, 355, 70).contains(sf::Mouse::getPosition(win)))
            {
                Bcontinue.getFillColor() != sf::Color::Yellow ? SMenuP.setPosition(130.f, 330.f) : void(),
                        Bcontinue.setFillColor(sf::Color::Yellow), Bcontrol.setFillColor(sf::Color::White),
                        Bmenu.setFillColor(sf::Color::White);
            }
            else if (sf::IntRect(185, 400, 330, 70).contains(sf::Mouse::getPosition(win)))
            {
                Bcontrol.getFillColor() != sf::Color::Yellow ? SMenuP.setPosition(145.f, 430.f) : void(),
                        Bcontinue.setFillColor(sf::Color::White), Bcontrol.setFillColor(sf::Color::Yellow),
                        Bmenu.setFillColor(sf::Color::White);
            }
            else if (sf::IntRect(250, 500, 160, 70).contains(sf::Mouse::getPosition(win)))
            {
                Bmenu.getFillColor() != sf::Color::Yellow ? SMenuP.setPosition(210.f, 530.f) : void(),
                        Bcontinue.setFillColor(sf::Color::White), Bcontrol.setFillColor(sf::Color::White),
                        Bmenu.setFillColor(sf::Color::Yellow);
            }
            else
            {
                SMenuP.setPosition(-30.f, -30.f), Bcontinue.setFillColor(sf::Color::White),
                        Bcontrol.setFillColor(sf::Color::White), Bmenu.setFillColor(sf::Color::White);
            }
            if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
            {
                if (Bcontinue.getFillColor() == sf::Color::Yellow)
                ...
                if (Bmenu.getFillColor() == sf::Color::Yellow)
                {
                    isEscape = true;  SMenuP.setPosition(-30.f, -30.f);
                    return;
                }
            }
        }
...
void Game::Play(sf::RenderWindow& win)
{// игровой процесс
    sf::Clock clock; Setup(true, false);
    while (!isEscape && win.isOpen())
    {
        if (gameOver && deathPacman)
        {
            isPause = true, time_passed = 0, gameOver = false;
        }
        ...
        while (win.pollEvent(event))
        {
            if (event.type == sf::Event::KeyPressed)
            {
                ...
                if (event.key.code == sf::Keyboard::Escape && second_pause == 4 && !deathPacman)
                {
                    SemiMenu(win);
                }
                if (event.key.code == sf::Keyboard::Space && second_pause == 4 && !deathPacman)
                {
                    isPause = !isPause;
                }
            }
        ...
        if (!isPause && second_pause == 4)
        {
            ...
            for (short i = 0; i < 4; ++i)
            {
                ghosts[i].move(pacman, dec_arr, dis_arr, map, score, lenght, killed_count, second_scared, level, GRedX, GRedY, Plast_dir, gameOver, deathPacman);
            }
        }
        Draw(win);
        if (!isEscape) win.display();
        fps_value = short(1 / (clock.getElapsedTime().asSeconds()));
        time_passed > 1000 ? (time_passed = 0, (second_pause == 4 ? 0 : ++second_pause),
                (deathPacman ? ++second_deathP : 0), (second_scared == 0 ? 0 : --second_scared),
                second_fruit == 0 ? 0 : --second_fruit) : time_passed += clock.getElapsedTime().asMilliseconds();
    }
